// The JavaScript runner for the backend test corpus. It reads an input file, runs the generated scanner and the
...
function appendScannerTrace(lines, source, inputPath) {
    ...
    const endPosition = scanner.position(scanner.byteOffset());
    const endLocation = `${endPosition.line}:${endPosition.column}`.padEnd(7);
...
function appendParserTrace(lines, source, inputPath) {
...
function appendTreeNode(lines, scanner, node, depth) {
...
function appendTreeTrace(lines, source, inputPath) {
...
function writeTrace(fileName, produce) {
    const lines = [];
    try {
        produce(lines);
    } catch (error) {
        process.stderr.write(`producing ${fileName} failed: ${error.stack ?? error}\n`);
    }
...
// readFileSync without an encoding returns a Buffer, which is a Uint8Array and therefore what the generated scanner
// wants. Handing it a string would be the classic mistake this harness exists to catch.
