...
// for propagating sets correctly across a directed graph which might contain loops and shortcuts.
//
// The algorithm does not care what the bits of the propagated sets mean. IELR(1) uses it with sets of terminals, when
// propagating goto follow sets along the goto follows relations, and with sets of kernel items, when propagating the
// follow kernel items of definition 3.16 along the goto follows internal relation.
type DigraphAlgorithm struct {
	// sets holds the sets being propagated, indexed by goto index. The caller owns this slice and seeds it with the
	// direct contributions before calling Execute. The algorithm merges the sets along the relation in place, so after
	// Execute the slice holds the fully propagated sets, which is the reflexive transitive closure of the relation
	// applied to the seed.
	sets []utils.Bitset
	...
	//                  own depth.
	//   - math.MaxInt: the goto's component has been closed; its set is final and it is ignored from then on.
	processed []int
}

// NewDigraphAlgorithm creates a new instances for algorithm digraph. The sets slice is indexed by goto index, owned
...
func NewDigraphAlgorithm(
	...
	successorGotoIdxOffsets := make([]int, len(sets)+1)
	...
	nextSuccessorSlot := make([]int, len(sets))
...
func (d *DigraphAlgorithm) traverse(gotoIdx int) {
	...
	for _, toIdx := range d.successorGotoIdxs[d.successorGotoIdxOffsets[gotoIdx]:d.successorGotoIdxOffsets[gotoIdx+1]] {
		...
		d.sets[gotoIdx].Merge(&d.sets[toIdx])
	}
	if d.processed[gotoIdx] == currDepth {
		for {
			topOfStack := d.gotoIdxWorkStack.Top()
			d.processed[topOfStack] = math.MaxInt
			// All members of a strongly connected component share the same set, which is fully accumulated in the root
			// of the component (gotoIdx). Copy it into each member, otherwise members other than the root keep an
			// incomplete set. This is the "F(Top of S) <- F x" step of the Digraph algorithm by DeRemer and Pennello.
			d.sets[topOfStack].Merge(&d.sets[gotoIdx])
