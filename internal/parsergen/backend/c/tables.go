...
// the Go backend emits, differing only in the type a table is given, which is a C one here.
...
func NewTables(parser backend.Parser, config Config) Tables {
	return table.NewTables(parser, table.TablesOptions{
		NewIntArray: utils.NewCIntArray,
		UintType:    utils.CUintType,
		TerminalName: func(symbol frontend.Symbol) string {
			return terminalName(config.Prefix, symbol)
		},
	})
}
