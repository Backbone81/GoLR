...
// data the Go backend emits, differing only in the type a table is given, which is a C one here.
type Tables struct {
...
func NewTables(parser backend.Parser, config Config) Tables {
	...
	columnType := utils.CUintType(noColumn)
	...
	return Tables{
		TerminalColumnByToken: newTerminalColumnByToken(parser, config),

		ActionBase:  utils.NewCIntArray(compressed.Actions.Base),
		ActionNext:  utils.NewCIntArray(table.FillHoles(compressed.Actions.Next)),
		...
		DefaultActionByState: utils.NewCIntArray(table.DefaultActions(compressed)),

		GotoBase: utils.NewCIntArray(compressed.Gotos.Base),
		GotoNext: utils.NewCIntArray(table.FillHoles(compressed.Gotos.Next)),
		GotoCheck: utils.NewTypedIntArray(
			utils.CUintType(nonterminalCount),
			table.FillChecks(compressed.Gotos.Check, nonterminalCount),
		),

		NoNonterminal:            nonterminalCount,
		DefaultGotoByNonterminal: utils.NewCIntArray(table.FillHoles(compressed.DefaultGotoByNonterminalIdx)),

		PopCountByProduction:    utils.NewCIntArray(table.PopCounts(parser)),
		NonterminalByProduction: utils.NewCIntArray(table.Nonterminals(parser)),
...
func newTerminalColumnByToken(parser backend.Parser, config Config) []TokenColumn {
	...
	for terminalIdx, terminal := range parser.Grammar.Terminals {
		result = append(result, TokenColumn{
			Name:   terminalName(config.Prefix, terminal),
