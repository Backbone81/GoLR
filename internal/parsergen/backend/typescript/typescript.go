...
// names none. It ends in .js rather than .ts because TypeScript resolves a specifier to the file the compiler produces.
...
type Config struct {
	...
	//
	// The parser needs the values the scanner gave the terminals and never learns them itself. TypeScript has a module
...
// FromParser writes the parser as TypeScript source code to the given writer. Returns an error if the TypeScript source
// code can not be encoded successfully.
func FromParser(writer io.Writer, parser backend.Parser, config Config) error {
	defer trace.StartRegion(context.TODO(), "GoLR: Parsergen: Backends: TypeScript: FromParser").End()
...
// ParserToFile writes the parser as TypeScript source code to the given file path. Returns an error if the file can not
...
func ParserToFile(filePath string, parser backend.Parser, config Config) (err error) {
	...
	if err != nil {
		return fmt.Errorf("creating the TypeScript file %q: %w", filePath, err)
	}
	defer func() {
...
// ParserToString returns the parser as TypeScript source code. Returns an error if the TypeScript source code can not
...
// underscore and a lowercase word among constants which are otherwise capitalized, which reads wrong in TypeScript
