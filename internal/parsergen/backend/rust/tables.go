...
func NewTables(parser backend.Parser) Tables {
	...
	columnType := utils.RustUintType(noColumn)
	...
	return Tables{
		...
		ActionBase:  utils.NewRustIntArray(compressed.Actions.Base),
		ActionNext:  utils.NewRustIntArray(table.FillHoles(compressed.Actions.Next)),
		...
		DefaultActionByState: utils.NewRustIntArray(table.DefaultActions(compressed)),

		GotoBase: utils.NewRustIntArray(compressed.Gotos.Base),
		GotoNext: utils.NewRustIntArray(table.FillHoles(compressed.Gotos.Next)),
		GotoCheck: utils.NewTypedIntArray(
			utils.RustUintType(nonterminalCount),
			table.FillChecks(compressed.Gotos.Check, nonterminalCount),
		),

		NoNonterminal:            nonterminalCount,
		DefaultGotoByNonterminal: utils.NewRustIntArray(table.FillHoles(compressed.DefaultGotoByNonterminalIdx)),

		PopCountByProduction:    utils.NewRustIntArray(table.PopCounts(parser)),
		NonterminalByProduction: utils.NewRustIntArray(table.Nonterminals(parser)),
...
// newTerminalColumnByToken returns one entry per terminal of the grammar, naming the token variant which stands for it
