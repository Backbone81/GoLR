...
// the Go table driven backend emits, differing only in the type a table is given, which is a Rust one here.
...
func NewTables(parser backend.Parser) Tables {
	return table.NewTables(parser, table.TablesOptions{
		NewIntArray:  utils.NewRustIntArray,
		UintType:     utils.RustUintType,
