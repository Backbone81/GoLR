...
import (
	...
	"github.com/backbone81/golr/internal/parsergen/backend/kotlin"
...
var _ = Describe("FromParser", func() {
	for wellKnownGrammar, parser := range ielr1golrcore.WellKnownParsers() {
		It("emits a parser for the "+wellKnownGrammar.Title+" grammar", func() {
			Expect(kotlin.FromParser(io.Discard, parser, kotlin.Config{
				PackageName: "parser",
			})).To(Succeed())
		})
	}
...
func BenchmarkFromParser(b *testing.B) {
	for wellKnownGrammar, parser := range ielr1golrcore.WellKnownParsers() {
		b.Run(wellKnownGrammar.Title, func(b *testing.B) {
			for b.Loop() {
				if err := kotlin.FromParser(io.Discard, parser, kotlin.Config{
					PackageName: "parser",
				}); err != nil {
					b.Fatal(err)
				}
