...
type Config struct {
	// PackageName is the Kotlin package the generated parser is declared in.
	//
	// It has to be the one the scanner was generated into. The parser needs the token type the scanner declares and the
	// scanner interface it reads the tokens through, and Kotlin resolves both by their package.
	PackageName string
}
...
// FromParser writes the parser as Kotlin source code to the given writer. Returns an error if the Kotlin source code
// can not be encoded successfully.
func FromParser(writer io.Writer, parser backend.Parser, config Config) error {
	defer trace.StartRegion(context.TODO(), "GoLR: Parsergen: Backends: Kotlin: FromParser").End()
...
// ParserToFile writes the parser as Kotlin source code to the given file path. Returns an error if the file can not be
...
func ParserToFile(filePath string, parser backend.Parser, config Config) (err error) {
	...
	if err != nil {
		return fmt.Errorf("creating the Kotlin file %q: %w", filePath, err)
	}
	defer func() {
...
func terminalName(symbol frontend.Symbol) string {
	...
	return utils.KotlinConstantName("Token" + utils.GoIdentifier(symbol.Name))
}
...
func nonterminalName(symbol frontend.Symbol) string {
	...
	return utils.KotlinConstantName("Nonterminal" + utils.GoIdentifier(symbol.Name))
}
