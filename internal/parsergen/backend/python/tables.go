...
func NewTables(parser backend.Parser) Tables {
	...
	noColumn := table.TerminalColumn(len(parser.Grammar.Terminals)-1) + 1
	...
	return Tables{
		...
		ActionBase:  utils.NewPythonIntArray(compressed.Actions.Base),
		ActionNext:  utils.NewPythonIntArray(table.FillHoles(compressed.Actions.Next)),
		ActionCheck: utils.NewPythonIntArray(table.FillChecks(compressed.Actions.Check, noColumn)),

		NoColumn:         noColumn,
		NoTerminalColumn: table.NoTerminalColumn,

		DefaultActionByState: utils.NewPythonIntArray(table.DefaultActions(compressed)),

		GotoBase:  utils.NewPythonIntArray(compressed.Gotos.Base),
		GotoNext:  utils.NewPythonIntArray(table.FillHoles(compressed.Gotos.Next)),
		GotoCheck: utils.NewPythonIntArray(table.FillChecks(compressed.Gotos.Check, nonterminalCount)),

		NoNonterminal:            nonterminalCount,
		DefaultGotoByNonterminal: utils.NewPythonIntArray(table.FillHoles(compressed.DefaultGotoByNonterminalIdx)),

		PopCountByProduction:    utils.NewPythonIntArray(table.PopCounts(parser)),
		NonterminalByProduction: utils.NewPythonIntArray(table.Nonterminals(parser)),
...
// newTerminalColumnByToken returns one entry per terminal of the grammar, naming the token member which stands for it
