...
// decisions for it.
//
// It carries the name of the token member rather than its value, because the value belongs to the scanner and this
// backend never learns it. The generated lookup is keyed by the members themselves, which is what keeps the two
// generators free of a numbering they would both have to agree on, see table.NoTerminalColumn.
type TokenColumn struct {
	// Name is the name of the token member.
	Name string
