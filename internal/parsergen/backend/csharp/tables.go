...
// the Go table driven backend emits, differing only in the type a table is given, which is a C# one here.
type Tables struct {
...
func NewTables(parser backend.Parser) Tables {
	...
	columnType := utils.CSharpIntType(noColumn)
	...
	return Tables{
		...
		ActionBase:  utils.NewCSharpIntArray(compressed.Actions.Base),
		ActionNext:  utils.NewCSharpIntArray(table.FillHoles(compressed.Actions.Next)),
		...
		DefaultActionByState: utils.NewCSharpIntArray(table.DefaultActions(compressed)),

		GotoBase: utils.NewCSharpIntArray(compressed.Gotos.Base),
		GotoNext: utils.NewCSharpIntArray(table.FillHoles(compressed.Gotos.Next)),
		GotoCheck: utils.NewTypedIntArray(
			utils.CSharpIntType(nonterminalCount),
			table.FillChecks(compressed.Gotos.Check, nonterminalCount),
		),

		NoNonterminal:            nonterminalCount,
		DefaultGotoByNonterminal: utils.NewCSharpIntArray(table.FillHoles(compressed.DefaultGotoByNonterminalIdx)),
