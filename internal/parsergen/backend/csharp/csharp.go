...
type Config struct {
	...
	//
	// It has to be the one the scanner was generated into. The parser needs the values the scanner gave the terminals
	// and the scanner interface it reads them through, and C# resolves both by namespace rather than by file.
	Namespace string
}
...
func FromParser(writer io.Writer, parser backend.Parser, config Config) error {
	defer trace.StartRegion(context.TODO(), "GoLR: Parsergen: Backends: CSharp: FromParser").End()
