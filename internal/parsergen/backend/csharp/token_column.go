...
//
// C# has no counterpart to the Go composite literal with named indexes, which the Go backend writes the same table as,
// so the generated code sizes the array to the highest token it names instead of leaving that to the compiler.
