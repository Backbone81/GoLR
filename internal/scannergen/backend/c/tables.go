...
type Tables struct {
	...
	// AcceptTokenByState holds, for every state, the name of the enumerator the state accepts, or the name of the
	...
	// table a token a reader of the generated scanner can follow. C leaves the width of an enumeration to the
	// implementation, so the table costs an int per state rather than the byte the numbers would need.
...
func NewTables(dfa backend.DFA, config Config) Tables {
	...
	classType := utils.CUintType(classCount)
	...
	for stateIdx, ruleIdx := range compressed.AcceptRuleIdxByStateIdx {
		if ruleIdx == table.NoRule {
			acceptTokenByState[stateIdx] = utils.CConstantName(config.Prefix, "TokenInvalidToken")
			continue
		}
		acceptTokenByState[stateIdx] = tokenName(config.Prefix, dfa.Rules[ruleIdx])
	}

	return Tables{
		ByteClassByByte: utils.NewTypedIntArray(classType, byteClassByByte),
		TransitionBase:  utils.NewCIntArray(compressed.Transitions.Base),
		TransitionNext:  utils.NewCIntArray(transitionNext),
