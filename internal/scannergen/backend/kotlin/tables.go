...
// tokenTypeName is the Kotlin type the accept table holds.
...
type Tables struct {
	...
	// class.
	TransitionNext utils.KotlinTable
	...
	TransitionCheck utils.KotlinTable
	...
	// AcceptTokenByState holds, for every state, the token the state accepts, or the invalid token for a state which
	// does not accept.
	//
	// The names are written into the generated source instead of the numbers they stand for, which is what makes the
	// table an Array<Token> a reader of the generated scanner can follow.
	AcceptTokenByState utils.KotlinNameTable
}
...
func NewTables(dfa backend.DFA) Tables {
	...
	classType := utils.KotlinIntType(classCount)
	...
	return Tables{
		ByteClassByByte: utils.NewKotlinTable("byteClassByByte", utils.NewTypedIntArray(classType, byteClassByByte)),
		TransitionBase:  utils.NewKotlinTable("transitionBase", utils.NewKotlinIntArray(compressed.Transitions.Base)),
		TransitionNext:  utils.NewKotlinTable("transitionNext", utils.NewKotlinIntArray(transitionNext)),
		TransitionCheck: utils.NewKotlinTable("transitionCheck", utils.NewTypedIntArray(classType, transitionCheck)),

		NoByteClass: classCount,

		AcceptTokenByState: utils.NewKotlinNameTable("acceptTokenByState", tokenTypeName, acceptTokenByState),
	}
}
