...
//go:embed scanner.kt.template
var scannerTemplate string

var parsedTemplate = template.Must(template.New("scanner.kt.template").Funcs(template.FuncMap{
	"tokenName":    tokenName,
...
type Config struct {
	// PackageName is the Kotlin package the generated scanner is declared in.
	//
	// The generated parser has to be given the same one. Kotlin resolves a type by its package and the parser needs
	// both the token type and the scanner interface declared here.
	PackageName string
}
...
// FromDFA writes the DFA as Kotlin source code to the given writer. Returns an error if the Kotlin source code can not
// be encoded successfully.
func FromDFA(writer io.Writer, dfa backend.DFA, config Config) error {
	defer trace.StartRegion(context.TODO(), "GoLR: Scannergen: Backend: Kotlin: FromDFA").End()
...
// by follows the Go backends, so that it is the same in every language, spelled the way Kotlin spells an enum entry.
func tokenName(ruleIdx int, rule frontend.Rule) string {
	return utils.KotlinConstantName("Token" + utils.GoIdentifier(rule.Name))
}
