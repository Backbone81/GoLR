...
func NewTables(dfa backend.DFA) Tables {
	...
	classType := utils.CSharpIntType(classCount)
	...
	return Tables{
		ByteClassByByte: utils.NewCSharpIntArray(byteClassByByte),
		TransitionBase:  utils.NewCSharpIntArray(compressed.Transitions.Base),
		TransitionNext:  utils.NewCSharpIntArray(transitionNext),
