...
// invalidTokenName is the name of the enumerator the generated scanner uses when it has no token, which is what the
...
func NewTables(dfa backend.DFA) Tables {
	...
	classType := utils.CppUintType(classCount)
	...
	return Tables{
		...
		TransitionBase:  utils.NewCppIntArray(compressed.Transitions.Base),
		TransitionNext:  utils.NewCppIntArray(transitionNext),
