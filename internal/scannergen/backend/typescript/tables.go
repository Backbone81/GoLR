...
// NameArray is a table of symbolic names together with the typed array which holds their values. The names are written
// into the generated source instead of the numbers they stand for, because a lookup table of bare numbers is
// unreadable to anyone debugging the generated scanner.
...
// data the Go table driven backend emits, differing only in that a type here names a typed array constructor. Those are
// the ones TypeScript inherits from JavaScript unchanged, so the helpers which pick them are the JavaScript ones.
type Tables struct {
