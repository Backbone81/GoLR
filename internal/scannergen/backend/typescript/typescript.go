...
//go:embed scanner.ts.template
var scannerTemplate string

var parsedTemplate = template.Must(template.New("scanner.ts.template").Funcs(template.FuncMap{
	"tokenName":  tokenName,
...
// FromDFA writes the DFA as TypeScript source code to the given writer. Returns an error if the TypeScript source code
// can not be encoded successfully.
func FromDFA(writer io.Writer, dfa backend.DFA) error {
	defer trace.StartRegion(context.TODO(), "GoLR: Scannergen: Backend: TypeScript: FromDFA").End()
...
// DFAToFile writes the DFA as TypeScript source code to the given file path. Returns an error if the file can not be
// written or the TypeScript source code can not be encoded successfully.
...
// backends rather than any TypeScript convention, so that a token is called the same in every language a scanner is
// generated for.
...
// tokenValue returns the value the generated scanner gives the token of the rule with the given index. TypeScript has
