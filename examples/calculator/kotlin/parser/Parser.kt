...
typealias TraceFunc = (String) -> Unit

/** Parses the tokens of a scanner into a parse tree. */
class Parser {
    /** When not null, called with one line for every action the parser takes. Set it after `Parser()`. */
    var trace: TraceFunc? = null
     ...
     */
    private fun reduce(scanner: TokenSource, productionIdx: Int) {
        ...
        if (trace != null) {
            ...
            emitTrace(
                ...
                "REDUCE",
                reduceTracePayload(
                    Nonterminal.entries[nonterminal],
    ...
    private fun recoverFromError(scanner: TokenSource): Boolean {
        ...
        while (true) {
            ...
            if (trace != null) {
                emitTrace(scanner, "POP", symbolTraceName(nodeStack.last().symbol))
            }
    ...
    private fun emitTrace(scanner: TokenSource, keyword: String, payload: String) {
        ...
        sink(String.format(Locale.ROOT, "%-7s %-7s %s", location, keyword, payload))
    }

    /** Emits an ERROR line, marked suppressed while error recovery is not reporting to the caller. */
    private fun emitErrorTrace(scanner: TokenSource, detail: String) {
        val fullDetail = if (errorRecoveryShiftsRemaining != 0) "(suppressed) $detail" else detail
        emitTrace(scanner, "ERROR", fullDetail)
    }
}
...
private fun symbolTraceName(symbol: ParseSymbol): String =
    when (symbol) {
...
private fun terminalTraceName(terminal: Token): String =
    when (terminal) {
        Token.END_TOKEN -> "\$end"
...
private fun escapeLexeme(lexeme: ByteBuffer): String {
    ...
    while (lexeme.hasRemaining()) {
        ...
        when (value) {
            ...
            0x09 -> result.append("\\t")
            in 0x20..0x7e -> result.append(value.toChar())
