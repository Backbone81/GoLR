...
public:
    ...
    static constexpr ParseSymbol new_terminal(Token terminal) {
        return ParseSymbol(static_cast<std::uint32_t>(terminal));
    }

    /// Returns the symbol for a nonterminal.
    static constexpr ParseSymbol new_nonterminal(Nonterminal nonterminal) {
        return ParseSymbol(NONTERMINAL_BIT | static_cast<std::uint32_t>(nonterminal));
    }

    /// Reports whether the symbol holds a terminal, which it then writes to the parameter.
    constexpr bool try_get_terminal(Token& terminal) const {
        terminal = static_cast<Token>(value_ & SYMBOL_MASK);
        return (value_ & NONTERMINAL_BIT) == 0;
    }

    /// Reports whether the symbol holds a nonterminal, which it then writes to the parameter.
    constexpr bool try_get_nonterminal(Nonterminal& nonterminal) const {
        nonterminal = static_cast<Nonterminal>(value_ & SYMBOL_MASK);
        return (value_ & NONTERMINAL_BIT) != 0;
    }

    /// Reports whether the two symbols stand for the same terminal or nonterminal.
    friend constexpr bool operator==(ParseSymbol left, ParseSymbol right) { return left.value_ == right.value_; }

    /// Reports whether the two symbols stand for different terminals or nonterminals.
    friend constexpr bool operator!=(ParseSymbol left, ParseSymbol right) { return !(left == right); }
...
inline std::string to_string(ParseSymbol symbol) {
    Token terminal = Token::InvalidToken;
    if (symbol.try_get_terminal(terminal)) {
        return std::string("terminal ") + to_string(terminal);
    }

    Nonterminal nonterminal{};
    symbol.try_get_nonterminal(nonterminal);
    return std::string("nonterminal ") + to_string(nonterminal);
}
...
struct ParseError {
    ...
    ParseError(std::string reason, bool is_syntax_error, const TokenSkipperScanner& scanner)
...
public:
    ...
    ParseResult parse(TokenSkipperScanner& scanner) {
        scanner_ = &scanner;
        ...
        while (true) {
            ...
            const StepOutcome outcome = step(error);
            ...
            ParseError failure = std::move(*error);
...
private:
    ...
    static constexpr std::size_t terminal_column(Token token) {
        switch (token) {
        case Token::EndToken:
            return 1;
    ...
    static std::optional<std::size_t> error_shift_state(std::size_t state) {
    ...
    StepOutcome step(std::optional<ParseError>& error) {
        const Token terminal = scanner_->token();
        ...
        case ACTION_KIND_SHIFT:
            ...
            node_stack_.push_back(ParseNode{ParseSymbol::new_terminal(terminal), scanner_->lexeme(), {}});
            scanner_->next();
            if (error_recovery_shifts_remaining_ > 0) {
        ...
        case ACTION_KIND_ERROR:
            error.emplace(std::string("unexpected token ") + to_string(terminal), true, *scanner_);
    ...
    bool recover_from_error() {
        if (error_recovery_shifts_remaining_ == ERROR_RECOVERY_SHIFTS) {
            ...
            if (scanner_->token() == Token::EndToken) {
        ...
        while (true) {
            ...
            if (next_state.has_value()) {
                ...
                node_stack_.push_back(
                    ParseNode{ParseSymbol::new_terminal(Token::ErrorToken), std::string_view(), {}});
                return true;
            }
    ...
    std::size_t current_state() const { return state_stack_.back(); }

    /// The scanner of the running parse.
    TokenSkipperScanner* scanner_ = nullptr;
