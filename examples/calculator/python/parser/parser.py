...
from .scanner import Token, TokenSkipperScanner, token_to_string
...
_NONTERMINAL_NAMES: dict[Nonterminal, str] = {
...
def nonterminal_to_string(nonterminal: Nonterminal) -> str:
    """Returns the name of the nonterminal, as the grammar spells it."""
    return _NONTERMINAL_NAMES.get(nonterminal, "unknown")
...
class TerminalSymbol:
    ...
    def __str__(self) -> str:
        ...
        return f"terminal {token_to_string(self.token)}"
...
class NonterminalSymbol:
    ...
    def __str__(self) -> str:
        """Returns what the symbol is and the name it goes by."""
        return f"nonterminal {nonterminal_to_string(self.nonterminal)}"
...
class ParseError(Exception):
    """A single parse error. A parse returns its errors instead of raising them, so reporting many of them is cheap.
    Raise one with `raise error`."""
    ...
    def __init__(self, reason: str, is_syntax_error: bool, scanner: TokenSkipperScanner) -> None:
        ...
        self.byte_offset = scanner.byte_offset()
        self.line = scanner.line()
        self.column = scanner.column()
        self.lexeme = scanner.lexeme()
        self.file_path = scanner.file_path()
...
"""Translates a token into the column of the action table which holds the decisions for it. A token the grammar does
not have is not in here."""

_ACTION_BASE: tuple[int, ...] = (
    9, 1, 9, 9, 0, 1, 4, 1, 9, 9, 9, 9, 1, 12, 12, 1,
    1,
)
"""Maps a state to the displacement of its row within `_ACTION_NEXT`."""

_ACTION_NEXT: tuple[int, ...] = (
    0, 28, 0, 0, 32, 36, 40, 44, 32, 36, 40, 44, 4, 48, 8, 0,
    0, 12, 40, 44, 0, 0, 0,
)
"""Holds the action of every entry. The action a state has for a column lives at `_ACTION_BASE[state] + column`, but
only if `_ACTION_CHECK` confirms the cell belongs to that column."""

_ACTION_CHECK: tuple[int, ...] = (
    11, 1, 11, 11, 4, 5, 6, 7, 4, 5, 6, 7, 3, 9, 5, 11,
    11, 8, 6, 7, 11, 11, 11,
)
"""Holds the column every cell of `_ACTION_NEXT` belongs to. A cell no state occupies holds 11,
which is one past the highest column in use and can never be asked for."""

_DEFAULT_ACTION_BY_STATE: tuple[int, ...] = (
...
_GOTO_BASE: tuple[int, ...] = (
    6, 6, 0, 1, 6, 6, 6, 6, 2, 3, 4, 5, 6, 6, 6, 6,
    6,
)
"""Maps a state to the displacement of its row within `_GOTO_NEXT`."""

_GOTO_NEXT: tuple[int, ...] = (
    0, 5, 6, 13, 14, 15, 16, 0,
)
"""Holds the state a goto leads to. The goto a state has for a nonterminal lives at `_GOTO_BASE[state] + nonterminal`,
but only if `_GOTO_CHECK` confirms the cell belongs to that nonterminal."""

_GOTO_CHECK: tuple[int, ...] = (
    2, 1, 1, 1, 1, 1, 1, 2,
)
"""Holds the nonterminal every cell of `_GOTO_NEXT` belongs to. A cell no state occupies holds
2, which is one past the highest nonterminal and can never be asked for."""

_DEFAULT_GOTO_BY_NONTERMINAL: tuple[int, ...] = (
    0, 4,
)
"""Holds the state a goto on a nonterminal leads to for every state `_GOTO_NEXT` has no entry for."""

_POP_COUNT_BY_PRODUCTION: tuple[int, ...] = (
    2, 1, 3, 3, 3, 3, 2, 3,
)
"""Holds how many symbols a reduction takes off the stacks, which is the length of the right hand side."""

_NONTERMINAL_BY_PRODUCTION: tuple[int, ...] = (
    0, 1, 1, 1, 1, 1, 1, 1,
)
...
def _terminal_column(token: Token) -> int:
    ...
    not have gets `_NO_TERMINAL_COLUMN`."""
...
class Parser:
    ...
    def parse(self, scanner: TokenSkipperScanner) -> ParseResult:
        ...
        while True:
            ...
            if outcome is _StepOutcome.ACCEPT:
                ...
                return ParseResult(self._node_stack[0], self._errors)
    ...
    def _step(self) -> _StepOutcome | ParseError:
        """Performs the one action the state on top of the stack takes for the current token. Returns an error only
        when the parse cannot go on."""
        terminal = self._scanner.token()
        ...
        if action_kind == _ACTION_KIND_SHIFT:
            ...
            self._node_stack.append(ParseNode(TerminalSymbol(terminal), self._scanner.lexeme(), []))
            self._scanner.next()
            if self._error_recovery_shifts_remaining > 0:
        ...
        if action_kind == _ACTION_KIND_ERROR:
            return ParseError(f"unexpected token {token_to_string(terminal)}", True, self._scanner)
        return ParseError(f"unexpected action {action} in state {state}", False, self._scanner)
    ...
    def _reduce(self, production_idx: int) -> None:
        ...
        self._node_stack.append(ParseNode(NonterminalSymbol(Nonterminal(nonterminal)), b"", children))
    ...
    def _recover_from_error(self) -> bool:
        ...
        if self._error_recovery_shifts_remaining == _ERROR_RECOVERY_SHIFTS:
            ...
            if self._scanner.token() == Token.END_TOKEN:
        ...
        while True:
            ...
            if next_state is not None:
                ...
                self._node_stack.append(ParseNode(TerminalSymbol(Token.ERROR_TOKEN), b"", []))
                return True
