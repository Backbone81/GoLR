...
_NONTERMINAL_NAMES: Final[dict[Nonterminal, str]] = {
    Nonterminal.ACCEPT_NONTERMINAL: "$accept",
...
class ParseNode:
    ...
    It carries the span of the source it covers, which `TokenSource.text` turns back into bytes and
    `TokenSource.position` into a file path, a line and a column.
...
_ERROR_TERMINAL_COLUMN: Final = 0
...
_TERMINAL_COLUMN_BY_TOKEN: Final[dict[Token, int]] = {
    Token.END_TOKEN: 1,
    Token.TOKEN_WHITESPACE: 2,
    Token.TOKEN_INTEGER: 3,
    Token.TOKEN_PLUS: 4,
    Token.TOKEN_MINUS: 5,
    Token.TOKEN_MULTIPLY: 6,
    Token.TOKEN_DIVIDE: 7,
    Token.TOKEN_LPAREN: 8,
    Token.TOKEN_RPAREN: 9,
    Token.TOKEN_UMINUS: 10,
}
...
_ACTION_CHECK: Final[tuple[int, ...]] = (
    11, 1, 11, 11, 4, 5, 6, 7, 4, 5, 6, 7, 3, 9, 5, 11,
    11, 8, 6, 7, 11, 11, 11,
)
"""Holds the column every cell of `_ACTION_NEXT` belongs to.

A cell no state occupies holds 11, which is one past the highest column in use and can never be
asked for.
"""

_DEFAULT_ACTION_BY_STATE: Final[tuple[int, ...]] = (
...
class Parser:
    ...
    """When not `None`, called with one line for every action the parser takes. Set it after `Parser()`."""
    ...
    def _reduce(self, scanner: TokenSource, production_idx: int) -> None:
        ...
        if self.trace is not None:
            ...
            self._emit_trace(
                scanner,
                "REDUCE",
                _reduce_trace_payload(Nonterminal(nonterminal), self._node_stack[len(self._node_stack) - pop_count:]),
            )
        ...
        byte_length = 0
        if pop_count != 0:
            last_child = self._node_stack[-1]
            byte_offset = self._node_stack[split_idx].byte_offset
        ...
        self._node_stack.append(
            ParseNode(
                NonterminalSymbol(Nonterminal(nonterminal)),
                byte_offset,
                byte_length,
                children=children,
    ...
    def _recover_from_error(self, scanner: TokenSource) -> bool:
        ...
        # node the round pops.
        dropped_offset = scanner.byte_offset
        dropped_length = 0
        ...
        while True:
            ...
            if next_state is not None:
                if self.trace is not None:
                    self._emit_trace(scanner, "RESYNC", "")
                if dropped_length == 0 and self._node_stack:
            ...
            if len(self._state_stack) == 1:
                ...
                return False
            if self.trace is not None:
                self._emit_trace(scanner, "POP", _symbol_trace_name(self._node_stack[-1].symbol))
            ...
            dropped_node = self._node_stack[-1]
    ...
    def _emit_trace(self, scanner: TokenSource, keyword: str, payload: str) -> None:
        """Hands one trace line to `self.trace`: the position of the current lookahead, the keyword, and an optional
        payload. The caller has already checked that `self.trace` is not `None`.
        """
        position = scanner.position(scanner.byte_offset)
        location = f"{position.line}:{position.column}"
        if payload == "":
            self.trace(f"{location:<7} {keyword}")
            return
        self.trace(f"{location:<7} {keyword:<7} {payload}")

    def _emit_error_trace(self, scanner: TokenSource, detail: str) -> None:
        """Emits an ERROR line, marked suppressed while error recovery is not reporting to the caller."""
        if self._error_recovery_shifts_remaining != 0:
            detail = "(suppressed) " + detail
        self._emit_trace(scanner, "ERROR", detail)


def _reduce_trace_payload(lhs: Nonterminal, rhs: list[ParseNode]) -> str:
    """Renders a reduction as `lhs => rhs`, or `lhs => ε` for an empty right hand side."""
    payload = str(lhs) + " =>"
    if not rhs:
        return payload + " ε"
    for node in rhs:
        payload += " " + _symbol_trace_name(node.symbol)
    return payload


def _symbol_trace_name(symbol: ParseSymbol) -> str:
    """The bare grammar name of a symbol, without the prefix its `__str__` adds."""
...
def _escape_lexeme(lexeme: bytes) -> str:
