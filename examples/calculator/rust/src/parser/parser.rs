...
/// A single node of the parse tree. It carries the span of the source it covers, which [`TokenSource::text`] turns
...
impl<'a> ParseError<'a> {
        ...
        S: TokenSource<'a> + ?Sized,
    {
        ...
        Self {
            ...
            file_path: position.file_path.to_owned(),
        }
    }
}
...
impl<'a, 't> ParseState<'a, 't> {
        ...
        match action & ACTION_KIND_MASK {
            ACTION_KIND_SHIFT => {
                ...
                self.node_stack.push(ParseNode {
                    ...
                    byte_length: scanner.lexeme().len(),
                    children: Vec::new(),
                    production: None,
                });
                scanner.next();
    ...
    fn reduce<S: TokenSource<'a> + ?Sized>(&mut self, scanner: &S, production_idx: usize) {
        ...
        let (byte_offset, byte_length) = if pop_count != 0 {
            let right_hand_side = &self.node_stack[self.node_stack.len() - pop_count..];
            let last_child = &right_hand_side[pop_count - 1];
            let byte_offset = right_hand_side[0].byte_offset;
            (
                byte_offset,
                last_child.byte_offset + last_child.byte_length - byte_offset,
            )
        } else if let Some(left) = self.node_stack.last() {
            ...
            (left.byte_offset + left.byte_length, 0)
        } else {
    ...
    fn recover_from_error<S: TokenSource<'a> + ?Sized>(&mut self, scanner: &mut S) -> bool {
        ...
        // node the round pops.
        let mut dropped_offset = scanner.byte_offset();
        let mut dropped_length = 0;
        ...
        if self.error_recovery_shifts_remaining == ERROR_RECOVERY_SHIFTS {
            ...
            dropped_length = scanner.lexeme().len();
            scanner.next();
        }
        ...
        loop {
            if let Some(next_state) = Self::error_shift_state(self.current_state()) {
                ...
                if dropped_length == 0 {
                    ...
                    if let Some(left) = self.node_stack.last() {
            ...
            let dropped_node = self
                .node_stack
                .last()
                .expect("a node for every state above the first");
