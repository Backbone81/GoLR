...
public readonly struct ParseSymbol : IEquatable<ParseSymbol>
{
    ...
    /// <summary>Returns what the symbol is and the name the grammar spells it with.</summary>
...
/// <summary>
/// A single node of the parse tree. It carries the span of the source it covers, which
/// <see cref="ITokenSource.Text"/> turns back into bytes and <see cref="ITokenSource.Position"/> into a file path, a
/// line and a column.
///
...
public sealed class ParseNode
{
    ...
    /// <param name="byteOffset">The start of the source the node covers.</param>
    /// <param name="byteLength">The number of bytes of the source the node covers.</param>
    ...
    public ParseNode(ParseSymbol symbol, int byteOffset, int byteLength, IReadOnlyList<ParseNode> children, Production? production)
    {
        Symbol = symbol;
    ...
    /// <summary>The start of the source this node covers, in bytes from the start of the source.</summary>
    public int ByteOffset { get; }

    /// <summary>The number of bytes of the source this node covers.</summary>
...
{
    /// <summary>An error in the input.</summary>
    Syntax,

    /// <summary>A defect of the parser itself, which ends the parse.</summary>
    Internal,
}

/// <summary>
/// A single parse error. Parse errors are returned by <see cref="Parser.Parse"/> rather than raised, so reporting many
/// of them costs nothing. This is no Exception, so it carries no stack trace. Throw one with
...
public sealed class ParseError
{
    ...
    /// <param name="kind">What the error is about.</param>
    /// <param name="scanner">The scanner the token and its position are taken from.</param>
    public ParseError(string reason, ErrorKind kind, ITokenSource scanner)
    {
        Reason = reason;
        Kind = kind;
    ...
    /// <summary>What the error is about.</summary>
...
public sealed class Parser
{
    ...
    /// <summary>
    /// When not null, called with one line for every action the parser takes. Set it after <c>new Parser()</c>.
    ///
    /// A debug aid; the line format carries no stability guarantee.
    /// </summary>
    ...
    public ParseResult Parse(ITokenSource scanner)
    {
        ...
        while (true)
        {
            StepResult result = Step(scanner, out ParseError? error);
            if (result == StepResult.Continue)
            {
                continue;
            }
            if (result == StepResult.Accept)
            {
    ...
    private StepResult Step(ITokenSource scanner, out ParseError? error)
    {
        error = null;
        Token terminal = scanner.Token;
        ...
        switch (action & ActionKindMask)
        {
            case ActionKindShift:
                if (Trace != null)
                {
                    EmitTrace(scanner, "SHIFT", $"{TerminalTraceName(terminal)} \"{EscapeLexeme(scanner.Lexeme.Span)}\"");
                }
                _stateStack.Add(action >> ActionKindBits);
                _nodeStack.Add(new ParseNode(
                    ParseSymbol.NewTerminal(terminal), scanner.ByteOffset, scanner.Lexeme.Length, Array.Empty<ParseNode>(), null));
                scanner.Next();
                if (_errorRecoveryShiftsRemaining > 0)
                {
            ...
            default:
                ...
                error = new ParseError($"unexpected action {action} in state {state}", ErrorKind.Internal, scanner);
                return StepResult.Failed;
        }
    }
    ...
    /// </summary>
    /// <param name="scanner">The scanner the lookahead position is taken from.</param>
    /// <param name="productionIdx">The production to reduce by.</param>
    private void Reduce(ITokenSource scanner, int productionIdx)
    {
        ...
        if (Trace != null)
        {
            ...
            EmitTrace(scanner, "REDUCE", ReduceTracePayload(
                (Nonterminal)nonterminal, _nodeStack.GetRange(_nodeStack.Count - popCount, popCount)));
        }
        ...
        if (popCount != 0)
        {
            ParseNode lastChild = _nodeStack[^1];
            byteOffset = _nodeStack[_nodeStack.Count - popCount].ByteOffset;
    ...
    /// </summary>
    private bool RecoverFromError(ITokenSource scanner)
    {
        ...
        if (_errorRecoveryShiftsRemaining == ErrorRecoveryShifts)
        {
            ...
            if (Trace != null)
            {
                EmitTrace(scanner, "DISCARD", $"{TerminalTraceName(scanner.Token)} \"{EscapeLexeme(scanner.Lexeme.Span)}\"");
            }
        ...
        while (true)
        {
            ...
            if (nextState != NoErrorShiftState)
            {
                ...
                if (droppedLength == 0 && _nodeStack.Count != 0)
                {
            ...
            if (Trace != null)
            {
                EmitTrace(scanner, "POP", SymbolTraceName(_nodeStack[^1].Symbol));
            }
            ...
            ParseNode droppedNode = _nodeStack[^1];
    ...
    /// <summary>
    /// Hands one trace line to <see cref="Trace"/>: the position of the current lookahead, the keyword, and an optional
    /// payload. The caller has already checked that <see cref="Trace"/> is not null.
    /// </summary>
    /// <param name="scanner">The scanner the lookahead position is taken from.</param>
    /// <param name="keyword">The trace keyword.</param>
    /// <param name="payload">The payload, or an empty string for none.</param>
    private void EmitTrace(ITokenSource scanner, string keyword, string payload)
    {
        Position position = scanner.Position(scanner.ByteOffset);
        string location = $"{position.Line}:{position.Column}".PadRight(7);
        if (payload.Length == 0)
        {
            Trace!.Invoke($"{location} {keyword}");
            return;
        }
        Trace!.Invoke($"{location} {keyword.PadRight(7)} {payload}");
    }

    /// <summary>Emits an ERROR line, marked suppressed while error recovery is not reporting to the caller.</summary>
    /// <param name="scanner">The scanner the lookahead position is taken from.</param>
    /// <param name="detail">The error detail, without the suppressed marker in front of it.</param>
    private void EmitErrorTrace(ITokenSource scanner, string detail)
    {
        if (_errorRecoveryShiftsRemaining != 0)
        {
    ...
    /// <summary>Renders a reduction as "lhs => rhs", or "lhs => ε" for an empty right hand side.</summary>
    /// <param name="lhs">The nonterminal on the left hand side.</param>
    /// <param name="rhs">The nodes of the right hand side.</param>
    ...
    /// <summary>The bare grammar name of a symbol, without the prefix its ToString adds.</summary>
    ...
    /// <summary>Escapes the bytes of a lexeme for a trace line. The caller writes the quotes around the result.</summary>
    /// <param name="lexeme">The bytes to escape.</param>
