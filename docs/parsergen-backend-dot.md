...
A node is a state, labelled with its index and its kernel items, each with a `•` marking how far the parse has come in
the production. An edge is a transition, labelled with the symbol it is taken on: solid for a shift on a terminal,
dashed for a goto on a nonterminal. Reductions and their lookaheads are not in the graph.
